...
function Signup() {
    ...
    async function handleSignup() {
        try {
            ...
            const response = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/auth/signup`, data);
            setLoading(false)
    ...
    async function onSubmit(id: string | null, otp: number) {
        try {
            ...
            const response = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/auth/otp-verification`, data)
    ...
    return (
        ...
        <>
            {
                isLoggedIn ? (<Otp length={8} onSubmit={onSubmit} email={email} loadingState={loadingState}></Otp >) : (<div className='w-full flex justify-center items-center pt-[100px]'>
